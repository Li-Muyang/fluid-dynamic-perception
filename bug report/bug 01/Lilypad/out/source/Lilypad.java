...
float ratio = 0.5f;//ratio of h and a 
...
public void setup(){
  ...
  flow = new BDIM(n,m,0.5f,ellipse1,nu,true);
...
public void draw(){
  ...
  if (PApplet.parseInt(time) % 1000 == 0) countRotate = 0;
  if(countRotate<10){//rotate shall be smooth       
    ellipse1.rotate(PI/180*0.1f);//everytime rotate 0.1, in all 10 steps rotate 1
    countRotate += 1;
  }
  println(time);
...
class Bread extends Body {
  ...
  Bread( float x, float y, float _h, float _l1, float _l2, Window window) {
    ...
    for ( int i=m/2; i<m; i++ ) {//up straight
      add(xc.x+l2*(1-PApplet.parseFloat(i)/m), xc.y-h);
    }
    end(); // finalize shape
  }
}
